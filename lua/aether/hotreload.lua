...
-- External theme spec files written by theme generators (aether CLI, omadora).
...
local OMADORA_THEME_PATHS = {
	vim.fn.expand("~/.config/omadora/current/theme/neovim.lua"),
	vim.fn.expand("~/.local/state/omadora/current/theme/neovim.lua"),
}
local OMADORA_THEME_PATH_SET = {}
for _, path in ipairs(OMADORA_THEME_PATHS) do
	OMADORA_THEME_PATH_SET[path] = true
end
...
_G.__aether_hotreload_state = _G.__aether_hotreload_state
	or {
		did_setup = false,
...
--- Omadora paths delegate to userland on change instead of applying opts.
--- The user's LazyReload handler (e.g.
--- ~/.config/nvim/lua/plugins/omadora-theme-hotreload.lua) is the right
...
--- @param path string
--- @return boolean
local function is_omadora_theme_path(path)
	return OMADORA_THEME_PATH_SET[path] == true
end
...
--- such entry exists. This is how the omadora file signals which scheme
...
---
--- For the omadora file, prefer the file's aether-direct opts when its
--- first entry is aether (this is the omadora "aether-themed" case):
--- apply them directly so the omadora theme's bundled palette becomes
--- active. This matches the pre-refactor behavior the user relies on -
--- omadora's neovim.lua IS the source of truth for aether colors.
--- Falls back to colorscheme delegation only when no aether-direct
--- entry is present (e.g. hackerman omadora theme, where the first
...
local function on_external_path_changed(path)
	local is_omadora_path = is_omadora_theme_path(path)
	local opts = get_theme_opts_from_file(path)
	if opts then
		if not is_omadora_path and not is_aether_active() then
			return
		end
...
local function schedule_reload(path)
	...
	state.pending_reload_timers[path] = vim.defer_fn(function()
		...
		-- No is_aether_active() gate: the omadora path delegates to userland
...
local function start_fs_watch(path)
	...
	if ok == 0 then
		...
		vim.schedule(function()
			vim.notify(("aether.nvim: failed to watch %s (%s)"):format(path, err or "unknown"), vim.log.levels.WARN)
		end)
		handle:close()
	end
end
...
---   2. ~/.config/omadora/ itself - catches `omadora theme set <name>`
...
local function start_fs_watch_dir(dir, tracked_files, filter_basenames)
	...
	if ok == 0 then
		...
		vim.schedule(function()
			vim.notify(("aether.nvim: failed to watch dir %s (%s)"):format(dir, err or "unknown"), vim.log.levels.WARN)
		end)
		handle:close()
	end
end
...
--- ~/.config/omadora/current or ~/.local/state/omadora/current), return the
...
local function group_omadora_symlink_anchors(paths)
	local grouped = {}
	local marker = "/omadora/current/"
	for _, path in ipairs(paths) do
		local marker_start = path:find(marker, 1, true)
		if marker_start then
			local anchor = path:sub(1, marker_start + #"/omadora" - 1)
...
--- tools (aether, omadora). BufWritePost only fires for in-editor writes, so
...
---   3. Symlink-anchor watcher (e.g. ~/.config/omadora/) to catch the
---      `current` symlink being retargeted by `omadora theme set`.
...
local function setup_external_config_watcher()
	...
	for omadora_dir, omadora_paths in pairs(group_omadora_symlink_anchors(EXTERNAL_THEME_PATHS)) do
		start_fs_watch_dir(omadora_dir, omadora_paths, { "current" })
	end
end
...
local function setup_status_command()
	vim.api.nvim_create_user_command("AetherReloadStatus", function()
		...
		for _, path in ipairs(OMADORA_THEME_PATHS) do
			table.insert(
				lines,
				("  omadora target   = %s (%s)"):format(tostring(get_colorscheme_from_file(path)), path)
			)
		end
		...
		local function describe(path, kind)
			...
			table.insert(lines, ("  [%s] %s  %s  watching=%s"):format(kind, path, extra, tostring(active)))
		end
		...
		for omadora_dir in pairs(group_omadora_symlink_anchors(EXTERNAL_THEME_PATHS)) do
			describe(omadora_dir, "anchor")
		end
